...
[upgrade/support guide](docs/product/upgrades-and-support.md). Install the terminal package with `npm install -g praxrail`, then use `pxr start --model <model> --base-url <url>` to start the engine and open the interactive prompt. Type plain task requests directly, use prompt commands such as `pxr tasks`, and return to your shell by typing `pxr stop` inside the session. `pxr ask "..."` remains available for scripts. During source development, run `pnpm cli -- version` to exercise the CLI entry point.
