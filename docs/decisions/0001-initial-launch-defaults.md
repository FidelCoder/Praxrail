...
- Deployment shape: one control-plane service plus PostgreSQL.
...
The production host/provider, FiberPass repository URLs, authorized Telegram IDs,
and live credentials are deployment inputs. The service refuses to enable an
integration until those inputs validate.
